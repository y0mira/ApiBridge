...
const refName = (ref: unknown) =>
  typeof ref === 'string' ? safeName(ref.split('/').at(-1) ?? 'Unknown') : undefined
...
function tsSchema(schema: Schema | undefined, context: Context, stack: string[] = []): string {
  if (!schema) {
    context.warnings.add('缺少 schema，已使用 unknown。')
    return 'unknown'
  }
  const referenced = refName(schema.$ref)
  if (referenced) {
    if (stack.includes(referenced)) context.recursive.add(referenced)
    return referenced
  }
  let result: string
  if (schema.const !== undefined) result = JSON.stringify(schema.const)
  else if (Array.isArray(schema.enum))
    result = schema.enum.map((item) => JSON.stringify(item)).join(' | ') || 'never'
  else if (Array.isArray(schema.oneOf) || Array.isArray(schema.anyOf))
    result = ((schema.oneOf ?? schema.anyOf) as Schema[])
      .map((part) => tsSchema(part, context, stack))
      .join(' | ')
  else if (Array.isArray(schema.allOf))
    result = schema.allOf.map((part) => tsSchema(part as Schema, context, stack)).join(' & ')
  else if (schema.type === 'array' || schema.items)
    result = `Array<${tsSchema(schema.items as Schema | undefined, context, stack)}>`
  else if (
    schema.type === 'object' ||
    schema.properties ||
    ...
    const fields = Object.entries((schema.properties as Record<string, Schema>) ?? {})
      .sort(([a], [b]) => a.localeCompare(b))
      .map(
        ([name, child]) =>
          `${quote(name)}${required.has(name) ? '' : '?'}: ${tsSchema(child, context, stack)};`,
      )
    if (schema.additionalProperties === true) fields.push('[key: string]: unknown;')
    else if (schema.additionalProperties && typeof schema.additionalProperties === 'object')
      fields.push(
        `[key: string]: ${tsSchema(schema.additionalProperties as Schema, context, stack)};`,
      )
    result = `{ ${fields.join(' ')} }`
    ...
    context.warnings.add('存在无法精确表达的 schema，已使用 unknown。')
    result = 'unknown'
  }
...
function zodSchema(schema: Schema | undefined, context: Context, stack: string[] = []): string {
  if (!schema) {
    context.warnings.add('缺少 schema，已使用 z.unknown()。')
    return 'z.unknown()'
  }
  const referenced = refName(schema.$ref)
  if (referenced) {
  ...
  if (schema.const !== undefined) result = `z.literal(${JSON.stringify(schema.const)})`
    ...
    result =
      values.every((item) => typeof item === 'string') && values.length
        ? `z.enum([${values.map(quote).join(', ')}])`
        : values.length
          ? `z.union([${values.map((item) => `z.literal(${JSON.stringify(item)})`).join(', ')}])`
          : 'z.never()'
  } else if (Array.isArray(schema.oneOf) || Array.isArray(schema.anyOf))
    result = `z.union([${((schema.oneOf ?? schema.anyOf) as Schema[]).map((part) => zodSchema(part, context, stack)).join(', ')}])`
  else if (Array.isArray(schema.allOf))
    result = schema.allOf
      .map((part) => zodSchema(part as Schema, context, stack))
      .reduce((left, right) => `${left}.and(${right})`)
  else if (schema.type === 'array' || schema.items)
    result = `z.array(${zodSchema(schema.items as Schema | undefined, context, stack)})`
  else if (
    schema.type === 'object' ||
    schema.properties ||
    schema.additionalProperties !== undefined
    ...
    const fields = Object.entries((schema.properties as Record<string, Schema>) ?? {})
      ...
      .map(
        ([name, child]) =>
          `${quote(name)}: ${zodSchema(child, context, stack)}${required.has(name) ? '' : '.optional()'}`,
      )
    result = `z.object({ ${fields.join(', ')} })`
    if (schema.additionalProperties === true) result += '.catchall(z.unknown())'
    else if (schema.additionalProperties && typeof schema.additionalProperties === 'object')
      result += `.catchall(${zodSchema(schema.additionalProperties as Schema, context, stack)})`
    ...
    context.warnings.add('存在无法精确表达的 schema，已使用 z.unknown()。')
    result = 'z.unknown()'
  }
  ...
  if (Object.keys(schema).some((key) => !known.has(key)))
    context.warnings.add('部分 OpenAPI constraints 未映射到 Zod；请检查生成结果。')
  return isNullable(schema) && result !== 'z.null()' ? `${result}.nullable()` : result
}

function componentOrder(document: OpenApiDocument) {
  return Object.entries(document.components?.schemas ?? {}).sort(([a], [b]) => a.localeCompare(b))
}
function operationName(operation: Operation) {
  return safeName(operation.operationId ?? operation.id)
}
...
export function generateTypes(document: OpenApiDocument, operations: Operation[]): GeneratedFile {
  ...
  const components = componentOrder(document)
    .map(
      ([name, schema]) =>
        `export type ${safeName(name)} = ${tsSchema(schema, context, [safeName(name)])}`,
    )
    .join('\n\n')
  const operationTypes = operations
    .map((operation) => {
      ...
      const groups = ['path', 'query', 'header', 'cookie'].map((location) => {
        ...
        const body = parameters.length
          ? `{ ${parameters
              ...
              .map(
                (item) =>
                  `${quote(item.name)}${item.required ? '' : '?'}: ${tsSchema(item.schema, context)};`,
              )
              .join(' ')} }`
          : 'Record<string, never>'
      ...
      const responses = operation.responses.length
        ? operation.responses
            .map(
              (response) =>
                `{ status: ${/^\d+$/.test(response.status) ? response.status : quote(response.status)}; body: ${tsSchema(response.schema, context)} }`,
            )
            .join(' | ')
        : 'never'
...
export function generateZod(document: OpenApiDocument, operations: Operation[]): GeneratedFile {
  const context: Context = { warnings: new Set(), recursive: new Set() }
  const schemas = componentOrder(document)
    .map(([name, schema]) => {
    ...
    .join('\n\n')
  const operationSchemas = operations
    .map((operation) => {
      const name = operationName(operation)
      const parameters = operation.parameters
        ...
        .map(
          (item) =>
            `${quote(item.name)}: ${zodSchema(item.schema, context)}${item.required ? '' : '.optional()'}`,
        )
...
function exampleFor(
  ...
  if (ref) {
    ...
    return exampleFor(target as Schema | undefined, document, seen)
  }
  if (Array.isArray(schema.oneOf) && schema.oneOf.length)
    return exampleFor(schema.oneOf[0] as Schema, document, seen)
  if (Array.isArray(schema.anyOf) && schema.anyOf.length)
    return exampleFor(schema.anyOf[0] as Schema, document, seen)
  if (Array.isArray(schema.allOf))
    return Object.assign(
      {},
      ...schema.allOf.map((item) => exampleFor(item as Schema, document, new Set(seen))),
    )
  if (schema.type === 'array' || schema.items)
    return [exampleFor(schema.items as Schema, document, seen)]
  if (schema.type === 'object' || schema.properties)
    return Object.fromEntries(
      Object.entries((schema.properties as Record<string, Schema>) ?? {})
        .sort(([a], [b]) => a.localeCompare(b))
        .map(([key, child]) => [key, exampleFor(child, document, new Set(seen))]),
    )
  if (schema.type === 'string')
    return schema.format === 'date-time' ? '1970-01-01T00:00:00.000Z' : 'string'
...
export function generateMsw(document: OpenApiDocument, operations: Operation[]): GeneratedFile {
  const handlers = operations
    .map((operation) => {
      const response =
        operation.responses.find((item) => /^2\d\d$/.test(item.status)) ?? operation.responses[0]
...
export function generateAll(document: OpenApiDocument, operations: Operation[]) {
  return [
    generateTypes(document, operations),
    generateZod(document, operations),
    generateMsw(document, operations),
